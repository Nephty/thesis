"""
    We plot the lengths of these symmetric differences, with the Y-axis
    representing the size of the symmetric difference and the X-axis representing the index of each member set
    (ranging from 0 to N). Each vertical bar in the plot indicates the size of the symmetric difference between
    a specific set and the base member sets. Figure 3.23 presents the resulting plot.
...
plt.ylim(0, 300)
plt.xlabel('Index of member set')
plt.ylabel('Symmetric difference length')
plt.grid(axis='y')
plt.tight_layout()
plt.savefig('out/3_1_base_set_members_difference.png')