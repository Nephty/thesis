"""
This plot is not shown in the thesis.
"""
...
FROM (SELECT filename, GROUP_CONCAT(CONCAT(username, access_level) ORDER BY username) AS member_set
      FROM {members}
...
plt.grid(axis='y')
plt.tight_layout()
plt.savefig('out/5_1_base_set_members_difference_with_role.png')